...
import { POLICIES, getRiderById } from '../data/mockData';
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-surface-border bg-surface-muted">
                    {['Policy ID', 'Rider', 'Type', 'Shift Count', 'Premium Paid', 'Start Date', 'End Date', 'Status'].map(h => (
                ...
                <tbody className="divide-y divide-surface-border">
                  {POLICIES.map(p => {
                    const rider = getRiderById(p.riderId);
                    return (
                      <tr key={p.id} className="hover:bg-surface-muted transition-colors">
                        <td className="px-5 py-4 font-mono text-[12px] font-semibold text-primary">{p.id}</td>
                        <td className="px-5 py-4">
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              ...
                              <p className="text-[10px] text-on-surface-variant">{p.riderId}</p>
                            </div>
                          </div>
                        </td>
                        <td className="px-5 py-4 text-[13px] text-on-surface">{p.type}</td>
                        <td className="px-5 py-4 text-[13px] font-semibold text-on-surface">{p.shiftCount}</td>
                        <td className="px-5 py-4 text-[13px] font-bold text-status-safe">₹{p.premiumPaid.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
                        <td className="px-5 py-4 text-[13px] text-on-surface">{p.startDate}</td>
                        <td className="px-5 py-4 text-[13px] text-on-surface">{p.endDate}</td>
