...
import { SHIFTS, getRiderById } from '../data/mockData';
        ...
        <main className="flex-1 p-6">
          <div className="flex justify-between items-end mb-6">
            <div>
              ...
              <p className="text-[14px] text-on-surface-variant mt-1">
                <span className="text-status-safe font-semibold">{activeCount} active</span> · {SHIFTS.length} total shifts today
              </p>
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            {[
              ...
              { label: 'Ended',   value: SHIFTS.filter(s => s.status === 'ENDED').length,  color: 'text-on-surface-variant', dot: 'bg-outline' },
              { label: 'High Risk', value: SHIFTS.filter(s => s.riskLevel === 'HIGH').length, color: 'text-status-emergency', dot: 'bg-status-emergency' },
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-surface-border bg-surface-muted">
                    {['Shift ID', 'Rider', 'Start', 'Duration', 'Distance', 'Risk', 'Status', 'Premium', 'Coverage'].map(h => (
                ...
                <tbody className="divide-y divide-surface-border">
                  {SHIFTS.map(s => {
                    const rider = getRiderById(s.riderId);
                    return (
                      <tr key={s.id} className="hover:bg-surface-muted transition-colors">
                        ...
                        <td className="px-5 py-4 whitespace-nowrap">
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              ...
                              <p className="text-[10px] text-on-surface-variant">{s.riderId}</p>
                            </div>
                          </div>
                        </td>
                        <td className="px-5 py-4 whitespace-nowrap text-[12px] text-on-surface">{s.start?.slice(11, 16)}</td>
                        <td className="px-5 py-4 whitespace-nowrap text-[12px] text-on-surface">{s.duration}</td>
                        <td className="px-5 py-4 whitespace-nowrap text-[12px] text-on-surface">{s.distance}</td>
                        <td className="px-5 py-4 whitespace-nowrap"><StatusBadge status={s.riskLevel} size="sm" /></td>
                        <td className="px-5 py-4 whitespace-nowrap">
                          <div className="flex items-center gap-2">
                        ...
                        <td className="px-5 py-4 whitespace-nowrap text-[13px] font-bold text-status-safe">₹{s.premium?.toFixed(2)}</td>
                        <td className="px-5 py-4 whitespace-nowrap"><StatusBadge status={s.coverageStatus} size="sm" /></td>
                      </tr>
                    );
                  })}
                </tbody>
